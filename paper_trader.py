...
class PaperTrader:
    ...
    def buy(
        ...
        if not is_market_open():
            logger.warning(f"Cannot buy {symbol}: forex market closed")
            return None
        ...
        if quantity <= 0:
            logger.warning(
                ...
                f"(need ${float(fill_price):.2f}, have ${float(self.portfolio.cash):.2f})"
            )
            return None
        ...
        logger.info(f"BUY {quantity}x {symbol} @ ${float(fill_price):.2f} = ${float(total_cost):.2f}")
        return order
    ...
    def sell(self, symbol: str, price: float | Decimal, quantity: int | None = None) -> Order | None:
        ...
        self.refresh_portfolio()
        if not is_market_open():
            logger.warning(f"Cannot sell {symbol}: forex market closed")
            return None
        ...
        if self._disk_sync_enabled:
            self.portfolio.save(self.filepath)
        pnl_str = f"+${float(pnl):.2f}" if pnl >= Decimal('0') else f"-${abs(float(pnl)):.2f}"
        logger.info(f"SELL {quantity}x {symbol} @ ${float(fill_price):.2f} | P&L: {pnl_str}")
        return order
    ...
    def check_stop_loss_take_profit(self, prices: dict[str, float]) -> list[Order]:
        ...
        self.refresh_portfolio()
        if not is_market_open():
            logger.info("Skipping stop-loss/take-profit checks: forex market closed")
            return []
        ...
        for symbol, pos in list(self.portfolio.positions.items()):
            ...
            if trailing_loss_pct <= -tightened_stop:
                logger.info(f"TRAILING STOP LOSS triggered for {symbol} ({float(trailing_loss_pct)*100:.1f}% from peak of ${float(pos.highest_price):.2f})")
