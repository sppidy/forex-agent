...
def _prepare_stock_summary(symbol: str, df: pd.DataFrame) -> str:
    ...
    summary = (
        f"Symbol: {symbol}\n"
        f"Price: {price:.5f} ({change_pct:+.2f}%)\n"
        f"5D Range: {low_5d:.5f}-{high_5d:.5f}\n"
...
async def analyze_batch_async(stock_data: list[tuple[str, pd.DataFrame]]) -> list[dict]:
    ...
    prompt = f"""You are a quantitative forex trading analyst. Analyze each currency pair and return trading signals.
...
PORTFOLIO: ${config.INITIAL_CAPITAL:,.0f} capital, max {config.MAX_POSITION_SIZE_PCT*100}% per position, {config.STOP_LOSS_PCT*100}% stop loss, {config.TAKE_PROFIT_PCT*100}% take profit.
...
- "symbol": string (use exact symbol with =X suffix: {symbol_list})
...
"""
    ...
    try:
        ...
        normalized = []
        if results:
            # Build lookup tolerant of AI stripping/adding =X suffix
            ...
            for item in results:
                ...
                by_symbol[sym_name.replace("=X", "")] = item
                if not sym_name.endswith("=X"):
                    by_symbol[sym_name + "=X"] = item
            for sym, _df in stock_data:
                raw = by_symbol.get(sym, by_symbol.get(sym.replace("=X", ""), {}))
...
2. Key risks
3. Strategy for ${config.INITIAL_CAPITAL:,.0f} capital"""
